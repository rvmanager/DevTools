  ...
  func run() throws {
    ...
    for var definition in analysisResult.definitions {
      ...
      if verbose {
        ...
        print("[DEBUG]   - Name: \(definition.name), Kind: \(definition.kind)")
        print(
          "[DEBUG]   - SwiftSyntax Location: line=\(definition.location.line), utf8Column=\(definition.location.utf8Column)"
        )
        ...
        if potentialMatches.isEmpty {
          ...
          for occ in potentialMatches {
            print(
              "[DEBUG]     - Symbol: \(occ.symbol.name), Kind: \(occ.symbol.kind), Location: line=\(occ.location.line), utf8Column=\(occ.location.utf8Column), Roles: \(occ.roles)"
            )
          }
        }
        print("[DEBUG] ----------------------------------------------------")
      ...
      // Try to find a match with the expected kind first
      if let potentialMatchWithKind = potentialMatches.first(where: {
        symbolKindsMatch(definition.kind, $0.symbol.kind)
      }) {
        match = potentialMatchWithKind
      }

      if match == nil {
        match = potentialMatches.first { !$0.roles.contains(.accessorOf) }
      }
      ...
      if let defOccurrence = match, !defOccurrence.symbol.usr.isEmpty {
        ...
        if verbose {
          log(
            "[HYDRATED] '\(definition.name)' -> Matched IndexStore Symbol: '\(defOccurrence.symbol.name)' (\(defOccurrence.symbol.kind)), USR: \(definition.usr ?? "N/A")"
          )
        }
      } else {
        unhydratedCount += 1
        log(
          "[HYDRATION WARNING] Could not find USR for \(definition.name) at \(definition.location.description)"
        )
      }
    }
