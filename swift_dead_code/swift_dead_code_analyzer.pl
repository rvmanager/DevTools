...
sub parse_calls_from_real_ast {
    ...
    if ($config{verbose} > 1) {
        ...
        print "  AST contains " . length($ast_content) . " characters\n";
    }
    ...
    # Debug: Count different AST patterns
    my @call_expr_matches = $ast_content =~ /\(call_expr/g;
    my @declref_matches = $ast_content =~ /\(declref_expr/g;
    
    if ($config{verbose} > 1) {
        print "  Found " . scalar(@call_expr_matches) . " call_expr patterns\n";
        print "  Found " . scalar(@declref_matches) . " declref_expr patterns\n";
    }
    
    # Look for call expressions with declref_expr - this finds direct function calls
    while ($ast_content =~ /\(call_expr[^)]*?\(declref_expr[^)]*?"([^"]+)"/gs) {
        my $called_function = $1;
        
        if ($config{verbose} > 2) {
            print "    Raw call_expr match: $called_function\n";
        }
        
        # Skip compiler internals and operators
        if ($called_function =~ /^\$|^<|operator|^print$|^fatalError$/) {
            if ($config{verbose} > 2) {
                print "      -> Skipped (compiler/builtin): $called_function\n";
            }
            next;
        }
        ...
        if ($config{verbose} > 2) {
            print "    Processed call to: $called_function" . ($callee_id ? " -> FOUND target: $callee_id" : " -> NO target found") . "\n";
        }
        
        if ($callee_id) {
            # Look for any function in the same file to use as caller
            my $caller_id;
            for my $func_id (keys %functions) {
                if ($functions{$func_id}->{file} eq $file_path) {
                    $caller_id = $func_id;
                    last;
                }
            }
            ...
            if ($caller_id) {
                ...
                if ($config{verbose} > 2) {
                    print "      -> Created edge: $caller_id -> $callee_id\n";
                }
            } else {
                if ($config{verbose} > 2) {
                    print "      -> No caller found in file $file_path\n";
                }
            }
        }
    }
    
    # Debug: Show some sample declref_expr patterns
    if ($config{verbose} > 1) {
        my @sample_declrefs = ();
        my $count = 0;
        while ($ast_content =~ /\(declref_expr[^)]*?"([^"]+)"/g && $count < 10) {
            push @sample_declrefs, $1;
            $count++;
        }
        print "  Sample declref_expr names: " . join(", ", @sample_declrefs) . "\n";
    }
    
    # Also look for simple declref_expr patterns (function references)
    while ($ast_content =~ /\(declref_expr[^)]*?"([^"]+)"/g) {
        my $referenced_function = $1;
        $declref_count++;
        
        # Skip obvious non-functions
        next if $referenced_function =~ /^\$|^<|^self$|^super$|^print$|^String$|^Int$|^Bool$/;
        next if $referenced_function =~ /^[A-Z][a-zA-Z]*$/ && length($referenced_function) < 10; # Skip type names
        
        # Look for functions that might be called
        my $callee_id = find_function_id_by_name($referenced_function);
        
        if ($callee_id && $config{verbose} > 2) {
            print "    Found declref to our function: $referenced_function -> $callee_id\n";
        }
        
        if ($callee_id) {
            # Create edges from any function in this file to this function
            for my $func_id (keys %functions) {
                if ($functions{$func_id}->{file} eq $file_path && $func_id ne $callee_id) {
                    push @{$call_graph{$func_id}}, $callee_id;
                    $edge_count++;
                    
                    if ($config{verbose} > 2) {
                        print "      -> Created declref edge: $func_id -> $callee_id\n";
                    }
                    last; # Only create one edge per reference
                }
            }
        }
    }
    ...
    if ($config{verbose} > 1) {
        print "  Found $call_count call_expr matches, $declref_count declref_expr matches\n";
        print "  Created $edge_count call graph edges\n";
    }
}
